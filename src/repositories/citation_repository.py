...
def get_citations():
    ...
    return [Citation(citation.id, citation.citation_type, citation.author, citation.title, citation.year) for citation in citations] 

def create_citation(citation_type, author, title, year):
    sql = text("INSERT INTO citations (citation_type, author, title, year) VALUES (:citation_type, :author, :title, :year)")
    db.session.execute(sql, {"citation_type": citation_type, "author": author, "title": title, "year":year})
    db.session.commit()
...
def get_citation_types():
    citation_types = {
        ...
        "inproceedings": {
            "required":["author", "title", "booktitle", "year"],
            "optional":["editor", "volume", "number", "series", "pages", "address", "month", "organization", "note"]
            },
        "manual": {
            "required":["title", "year"],
